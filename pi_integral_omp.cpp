...
{ 
  ...
  cin >> niter;
  cout << "Enter Threads ";
  cin >> threads;   //number of threads to parallize 

  omp_set_num_threads(threads);   //directive to set number of threads
  ...
  double sum = 0.0; 
  #pragma omp parallel for private ( x ) shared ( h, niter ) reduction ( +:sum )
